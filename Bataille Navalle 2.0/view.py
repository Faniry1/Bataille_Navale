...
def souris(event):
    x,y = event.x, event.y
    cadrillage(carte1,terrain1)
    for i in range(pixel//C):
            for j in range(pixel//C):
                if j*C<x<C+j*C and i*C<y<C+i*C:
                    terrain1.create_rectangle(j*C,i*C,C+j*C,i*C+C,outline="white",width=3)
    ...
    for i in range(pixel//C):
            for j in range(pixel//C):
                if j*C<x<C+j*C and i*C<y<C+i*C:
                    tir(carte2,i,j)
    cadrillage2()
    root.after(25, gagnant)
    root.after(25,tirAveugle)
    root.after(25, cadrillage1)

def gagnant():
    if bateauDansCarte(carte2)==True:
        gagne = Label(width=50,height=20,font="Arial 20 bold",text="YOU WIN",bg="lightgreen")
        gagne.pack(padx=20,pady=20)
        terrain1.destroy()
        terrain2.destroy()
    elif bateauDansCarte(carte1)==True:
        gagne = Label(width=30,height = 15,font="Arial 30 bold",text="YOU LOSE",bg="red2")
        gagne.pack(padx=20,pady=20)
        terrain1.destroy()
        terrain2.destroy()
