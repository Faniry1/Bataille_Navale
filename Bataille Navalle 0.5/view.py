...
global vert
global hori
vert=[PhotoImage(file = "porteavionv.png"),PhotoImage(file = "croiseurv.png"),
      PhotoImage(file = "contretorpilleurv.png"),PhotoImage(file = "sousmarinv.png"),
      PhotoImage(file = "torpilleurv.png"),PhotoImage(file = "r.png")]
hori=[PhotoImage(file = "porteavionh.png"),PhotoImage(file = "croiseurh.png"),
      PhotoImage(file = "contretorpilleurh.png"),PhotoImage(file = "sousmarinh.png"),
      PhotoImage(file = "torpilleurh.png"),PhotoImage(file = "r.png")]

# Image des bateaux couler
coulerv=[PhotoImage(file = "couler1v.png"),PhotoImage(file = "couler2v.png"),
         PhotoImage(file = "couler3v.png"),PhotoImage(file = "couler3v.png"),
         PhotoImage(file = "couler5v.png"),PhotoImage(file = "r.png")]
coulerh=[PhotoImage(file = "couler1h.png"),PhotoImage(file = "couler2h.png"),
         PhotoImage(file = "couler3h.png"),PhotoImage(file = "couler3h.png"),
         PhotoImage(file = "couler5h.png"),PhotoImage(file = "r.png")]

# Image explosion et rater
explosion2 = PhotoImage(file="explosion2.png")
rater = PhotoImage(file = "rater.png")
...
def plateau():
    ...
    bouton2.pack_forget()
    bouton3.pack_forget()
    boutbat.pack(side="right",padx=50)
...
# Création de bateau en fonction du compteur
def createboat():
    global compteur
    global boat
...
# Appel du terrein de l'ordinateur
def terrainOrdi():
        ...
        for j in range(Side//C):
            if j*C<x<C+j*C and i*C<y<C+i*C:
                if carte2[i][j] =='X' or carte2[i][j] =='T' :
                    ...
                    cnv2.create_image(j*C+25,i*C+25,image=rater)
                    tirSurOrdi('X',i,j)
                    eau.play()
                    tirvalide = True
                elif carte2[i][j] !='X' and carte2[i][j] !='.':
                    q = carte2[i][j]
                    cnv2.create_image(j*C+25,i*C+25,image=explosion2)
                    tirSurOrdi("T",i,j)
                    touchercouler2(q)
                    explosion.play()
                    tirvalide = True
...
def tir_ordi():
    ...
    if carte[x][y] == 'X' or carte[x][y] == 'T':
        ...
        cnv1.create_image(y*C+25,x*C+25,image = rater)
        tirSurJoueur('X',x,y)
        verif(a,b)
    elif carte[x][y] != ".":
        q = carte[x][y]
        cnv1.create_image(y*C+25,x*C+25,image=explosion2)
        tirSurJoueur('T',x,y)
        touchercouler1(q)
        verif(a,b)
...
# Bouton pour fermer
fermer = Button(root, font="Arial 15 bold",text="Fermer",
                command=quitter,width=10,bg="red")
...
# Bouton pour fermer
restart = Button(root, font="Arial 15 bold",text="Restart"
                 ,command=restart,width=10,bg="yellow")
...
def moved(event):
    ...
    if xboat-25<event.x<xboat+25 and yboat-25*long<event.y<yboat+25*long and sens=="verticale" and 25*long<event.y<510-25*long and 25<event.x<700-25:
...
Coop1 = [[None,None,""],[None,None,""],[None,None,""],[None,None,""],[None,None,""]]
def poser(event):
    ...
    for i in range(10):
        for j in range(10):
            if j*C<xboat<j*C+C and i*C<yboat<i*C+C and sens=="verticale" and colision2(i,j,long,"verticale")==False :
                ...
                if long%2 != 0:
                    Coop1[compteur][0],Coop1[compteur][1],Coop1[compteur][2] = j*C+25,i*C+25,"verticale"
                    boat = cnv1.create_image(j*C+25,i*C+25,image=vert[compteur])
                else:
                    Coop1[compteur][0],Coop1[compteur][1],Coop1[compteur][2] = j*C+25,i*C,"verticale"
            ...
            # Pareil qu'en haut       
                ...
                cnv1.delete(boat)
                if long%2 != 0:
                    Coop1[compteur][0],Coop1[compteur][1],Coop1[compteur][2] = j*C+25,i*C+25,"horizontale"
                    boat = cnv1.create_image(j*C+25,i*C+25,image=hori[compteur])
                else:
                    Coop1[compteur][0],Coop1[compteur][1],Coop1[compteur][2] = j*C,i*C+25,"horizontale"
...
# Faire apparaître un bateau couler de la carte du joueur
def touchercouler1(nbateau):
    if estdans(nbateau) == False:
        if Coop1[int(nbateau)-1][2] == "verticale":
            cnv1.create_image(Coop1[int(nbateau)-1][0],Coop1[int(nbateau)-1][1],image= coulerv[int(nbateau)-1])
        elif Coop1[int(nbateau)-1][2] == "horizontale":
            cnv1.create_image(Coop1[int(nbateau)-1][0],Coop1[int(nbateau)-1][1],image= coulerh[int(nbateau)-1])

# Faire apparaître un bateau couler de la carte de l'ordinateur
def touchercouler2(nbateau):
    if estdans2(nbateau) == False:
        if Coop2[int(nbateau)-1][2] == "verticale":
            cnv2.create_image(Coop2[int(nbateau)-1][0],Coop2[int(nbateau)-1][1],image= coulerv[int(nbateau)-1])
        elif Coop2[int(nbateau)-1][2] == "horizontale":
            cnv2.create_image(Coop2[int(nbateau)-1][0],Coop2[int(nbateau)-1][1],image= coulerh[int(nbateau)-1])